...
import { saveMobileAudioBlob, getMobileAudioBlobUrl, removeMobileAudioBlob } from './mobileAudioStore';
...
// 移动端（Capacitor）导入路径的固定根目录名：多文件选择没有真实目录结构。
const MOBILE_IMPORT_FOLDER_NAME = '设备文件';

// 移动端文件导入：复用系统文件选择器返回的 File 对象，音频 blob 持久化到 IndexedDB，
// 播放时从 blob 取 URL（getAudioFromLocalSong 的 mobile 分支）。
export async function importMobileFiles(files: File[]): Promise<LocalSong[]> {
    ...
    for (const file of audioFiles) {
        ...
        let embeddedMetadata: EmbeddedMetadata = {};
        try {
            embeddedMetadata = await extractEmbeddedMetadata(file, true);
        } catch (error) {
            console.warn(`[LocalMusic][MobileImport] Failed to parse metadata for ${file.name}:`, error);
        }
        ...
        let localCoverAssetId: string | undefined;
        let localCoverSource: LocalSong['localCoverSource'] = undefined;
        if (embeddedMetadata.cover) {
            const prepared = await prepareLocalCoverBlob(embeddedMetadata.cover);
            if (prepared?.assetId) {
                localCoverAssetId = prepared.assetId;
        ...
        const localSong: LocalSong = {
            ...
            fileSize: file.size,
            fileLastModified: file.lastModified,
            fileSignature: buildFileSignature(file.name, file.size, file.lastModified),
            mimeType: file.type,
            bitrate: embeddedMetadata.bitrate || 0,
            addedAt: Date.now(),
            ...
            replayGainAlbumGain: embeddedMetadata.replayGainAlbumGain,
            replayGainAlbumPeak: embeddedMetadata.replayGainAlbumPeak,
            folderName: MOBILE_IMPORT_FOLDER_NAME,
            isMobileImport: true,
        };

        await saveMobileAudioBlob(songId, file);
...
export async function getAudioFromLocalSong(song: LocalSong): Promise<string | null> {
    if (song.isMobileImport) {
        const url = await getMobileAudioBlobUrl(song.id);
        if (url) return url;
        console.warn(`[LocalMusic] Missing mobile audio blob for song ${song.id}. Re-import is required.`);
        return null;
    }
...
export async function deleteSongsByIds(songIds: string[]): Promise<void> {
    ...
    await Promise.all([
        ...
        ...uniqueSongIds.map(id => removeMobileAudioBlob(id)),
    ]);
