...
export function usePlaybackQueueController({
    ...
    const playSong = useCallback(async (
        ...
        // 汽水等 playback=false 的在线音源没有音频源，提前拦截并给出明确提示，避免走到
        // loadOnlineSongAudioSource 后被误报成「歌曲已下架」。
        if (!isLocal && !isNavidrome && !omni.canPlaySong(song)) {
            const sourceRef = getPlaybackSourceRef(song);
            const providerLabel = sourceRef.kind === 'online'
                ? omni.getProviderLabel(sourceRef.providerId)
                : '';
            setStatusMsg({ type: 'error', text: t('status.providerPlaybackUnsupported', { provider: providerLabel }) });
            setIsLyricsLoading(false);
            return;
        }
