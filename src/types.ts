...
/**
 * 交融 (Fusion) tuning. Blends the three lyric-PV visual vocabularies into one deterministic
 * program: newspaper print background (fume), per-glyph scale hierarchy + oversized decorative
 * copies (sonnet), and directional entrance with post-sung release tracking (tempera).
 */
export interface FusionTuning {
  /** Camera drift intensity, 0..2. */
  cameraIntensity: number;
  /** Hero/support glyph scale contrast, 1..2. */
  heroScale: number;
  /** Per-glyph entrance motion strength, 0..2. */
  glyphMotion: number;
  /** Post-sung release tracking spread, 0..2. */
  releaseSpread: number;
  /** Ink-reveal blur strength, 0..1. */
  inkReveal: number;
  /** Oversized decorative glyph copies behind/around the hero. */
  showDecor: boolean;
  /** Newspaper print halo / geometric line background. */
  showHalo: boolean;
  /** Render texture resolution multiplier, 0.5..4. */
  textureResolution: number;
}
...
export interface LocalSong {
  ...
  isMobileImport?: boolean; // True when imported via Capacitor file picker (blob persisted in mobile_audio_blobs)
