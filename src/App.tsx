...
import { useLandscapePlayback } from './hooks/useLandscapePlayback';
...
import { useQishuiLibrary } from './hooks/useQishuiLibrary';
...
export default function App() {
    ...
    const { isLandscape, toggleLandscape } = useLandscapePlayback();
    const { isTouch } = usePlatformCapabilities();
    ...
    const visualizerTunings = useMemo(() => ({
        ...
        fusion: fusionTuning,
    }), [cadenzaTuning, cappellaTuning, classicTuning, claddaghTuning, dioramaTuning, fumeTuning, fusionTuning, monetTuning, partitaTuning, pendoloTuning, sonnetTuning, temperaTuning, tiltTuning]);
    ...
    const {
        refresh: refreshQishuiLibrary,
        logout: logoutQishuiLibrary,
        checkLoginStatus: checkQishuiLoginStatus,
    } = useQishuiLibrary();
    const {
    ...
    const onlineProviderRefreshers = useMemo(() => ({
        ...
        qishui: refreshQishuiLibrary,
        qq: refreshQqLibrary,
    }), [refreshQishuiLibrary, refreshQqLibrary, refreshUserData]);
    const onlineProviderLogouts = useMemo(() => ({
        netease: handleLogout,
        qishui: logoutQishuiLibrary,
        qq: logoutQqLibrary,
    }), [handleLogout, logoutQishuiLibrary, logoutQqLibrary]);
    ...
    useEffect(() => {
        ...
        void refreshActiveProviderPlaylists().catch(async error => {
            ...
            if (providerId !== 'qishui' || !account?.user) return;

            const user = await checkQishuiLoginStatus();
            const refreshedAccount = useOnlineProviderAccountStore.getState().accounts.qishui;
            if (!user && refreshedAccount?.error === 'auth-required') {
                setStatusMsg({ type: 'error', text: t('status.loginExpired') });
            }
        });
    }, [checkQishuiLoginStatus, currentView, hasCollection, onlineProviderPlatform.activeProvider?.freshness, onlineProviderPlatform.activeProviderId, refreshActiveProviderPlaylists, setStatusMsg, t]);
