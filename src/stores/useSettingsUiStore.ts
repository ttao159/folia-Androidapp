...
const readStoredFusionTuning = (): FusionTuning => {
    if (typeof window === 'undefined') return DEFAULT_FUSION_TUNING;
    const saved = localStorage.getItem('fusion_tuning');
    if (!saved) return DEFAULT_FUSION_TUNING;
    try {
        const parsed = JSON.parse(saved) as Partial<FusionTuning>;
        return {
            cameraIntensity: resolvePendoloNumber(parsed.cameraIntensity, DEFAULT_FUSION_TUNING.cameraIntensity, 0, 2),
            heroScale: resolvePendoloNumber(parsed.heroScale, DEFAULT_FUSION_TUNING.heroScale, 1, 2),
            glyphMotion: resolvePendoloNumber(parsed.glyphMotion, DEFAULT_FUSION_TUNING.glyphMotion, 0, 2),
            releaseSpread: resolvePendoloNumber(parsed.releaseSpread, DEFAULT_FUSION_TUNING.releaseSpread, 0, 2),
            inkReveal: resolvePendoloNumber(parsed.inkReveal, DEFAULT_FUSION_TUNING.inkReveal, 0, 1),
            showDecor: typeof parsed.showDecor === 'boolean' ? parsed.showDecor : DEFAULT_FUSION_TUNING.showDecor,
            showHalo: typeof parsed.showHalo === 'boolean' ? parsed.showHalo : DEFAULT_FUSION_TUNING.showHalo,
            textureResolution: resolvePendoloNumber(parsed.textureResolution, DEFAULT_FUSION_TUNING.textureResolution, 0.5, 4),
        };
    } catch {
        return DEFAULT_FUSION_TUNING;
    }
};
...
export type SettingsUiState = {
    ...
    handleSetFusionTuning: (patch: Partial<FusionTuning>) => void;
    handleResetFusionTuning: () => void;
...
export const useSettingsUiStore = create<SettingsUiState>((set, get) => ({
    ...
    fusionTuning: readStoredFusionTuning(),
    ...
    handleSetFusionTuning: (patch: Partial<FusionTuning>) => {
        const prev = get().fusionTuning;
        const next: FusionTuning = {
            cameraIntensity: resolvePendoloNumber(patch.cameraIntensity, prev.cameraIntensity, 0, 2),
            heroScale: resolvePendoloNumber(patch.heroScale, prev.heroScale, 1, 2),
            glyphMotion: resolvePendoloNumber(patch.glyphMotion, prev.glyphMotion, 0, 2),
            releaseSpread: resolvePendoloNumber(patch.releaseSpread, prev.releaseSpread, 0, 2),
            inkReveal: resolvePendoloNumber(patch.inkReveal, prev.inkReveal, 0, 1),
            showDecor: typeof patch.showDecor === 'boolean' ? patch.showDecor : prev.showDecor,
            showHalo: typeof patch.showHalo === 'boolean' ? patch.showHalo : prev.showHalo,
            textureResolution: resolvePendoloNumber(patch.textureResolution, prev.textureResolution, 0.5, 4),
        };
        if (typeof window !== 'undefined') localStorage.setItem('fusion_tuning', JSON.stringify(next));
        set({ fusionTuning: next });
    },
    handleResetFusionTuning: () => {
        if (typeof window !== 'undefined') {
            localStorage.setItem('fusion_tuning', JSON.stringify(DEFAULT_FUSION_TUNING));
        }
        set({ fusionTuning: DEFAULT_FUSION_TUNING });
        notify(get, { type: 'info', text: i18n.t('notifications.fusionReset') });
    },
...
export const selectSettingsUiSnapshot = (state: SettingsUiState) => ({
    ...
    fusionTuning: state.fusionTuning,
    ...
    handleSetFusionTuning: state.handleSetFusionTuning,
    handleResetFusionTuning: state.handleResetFusionTuning,
