...
import { importFolder, importMobileFiles, resyncAllFolders, LOCAL_MUSIC_SCAN_PROGRESS_EVENT } from '../services/localMusicService';
...
import { pickMobileAudioFiles } from '../services/capacitor/filePicker';
...
const LOGIN_COPY_BY_PROVIDER: Record<string, { title: string; note: string }> = {
    qishui: { title: 'home.loginTitleQishui', note: 'home.loginNoteQishui' },
...
export const Grid3D: React.FC<Grid3DProps> = (props) => {
    ...
    const fetchRadioItems = async () => {
        ...
        try {
            ...
            // 汽水等 provider 不提供个人 FM 或每日推荐，返回空数组；空结果不渲染对应卡片，
            // 避免 radio 页出现无封面、无法打开的占位项。
            const radioItems: any[] = [];
            if (fmSongs.length > 0) {
                radioItems.push({
                    id: 'personal_fm',
                    name: t('home.personalFm'),
                    coverUrl: getSongCoverUrl(fmSongs[0], activeProviderId),
            ...
            radioItems.push(...recommendedItems);
    ...
    const handleFolderImport = async () => {
        ...
        try {
            if (isNative) {
                const files = await pickMobileAudioFiles();
                if (files.length === 0) return;
                const importedSongs = await importMobileFiles(files);
