...
import { Play, Pause, Repeat, Repeat1, RepeatOff,ChartBar, RotateCw, RotateCcw, EyeOff } from 'lucide-react';
...
const FloatingPlayerControls: React.FC<FloatingPlayerControlsProps> = ({
    ...
    const showLandscapeButton = isNative && !!onToggleLandscape;
    ...
    // 隐藏按钮仅在播放器（歌词背景）界面可用；首页悬浮条是导航入口，不应被收起。
    const handleHideProgressBar = currentView === 'player' ? () => setIsMinimized(true) : undefined;
        ...
        <>
            <motion.div
                className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-60 w-full flex justify-center transition-all duration-300 pointer-events-none
                    ${currentView === 'home' ? 'max-w-[calc(100vw-120px)] md:max-w-lg landscape:max-w-2xl' : 'max-w-lg px-4 landscape:max-w-2xl'}`}
                initial={false}
                animate={{
                    opacity: isHidden || isMinimized ? 0 : 1,
                    y: isHidden || isMinimized ? 24 : 0,
                    scale: isHidden || isMinimized ? 0.97 : 1,
                }}
                transition={{ duration: 0.26, ease: 'easeOut' }}
                style={{ pointerEvents: isHidden || isMinimized ? 'none' : 'auto' }}
                        ...
                        <motion.div layout transition={{ layout: CONTROL_LAYOUT_SPRING }} className="w-full">
                            {showExpanded ? (
                                <ExpandedView
                                    ...
                                    showLandscapeButton={showLandscapeButton}
                                    onHide={handleHideProgressBar}
                                />
                            ) : (
                                <CollapsedView
                                    currentTime={currentTime}
            ...
            {isMinimized && currentView === 'player' && (
                <div
                    className="fixed inset-0 z-50 cursor-pointer"
                    role="button"
                    aria-label={t('ui.showProgressBar')}
        ...
        <div className="grid w-full grid-cols-[1fr_auto_1fr] items-center gap-x-4 gap-y-2 sm:grid-cols-[auto_minmax(0,1fr)_auto]">
            ...
            <div className="col-span-3 row-start-1 min-w-0 px-2 sm:col-start-2 sm:col-span-1">
                {trackNavigation ? (
                    ...
                    <div className="truncate text-center text-sm landscape:text-base font-bold select-none" style={{ color: primaryColor }}>
            ...
            <button
                ...
                className={`col-start-2 row-start-2 flex h-12 w-12 landscape:h-14 landscape:w-14 shrink-0 items-center justify-center rounded-full border-none bg-(--text-primary) text-black shadow-lg transition-transform sm:col-start-1 sm:row-start-1 sm:row-span-2 ${controlsDisabled ? 'cursor-not-allowed opacity-45' : 'hover:scale-105'}`}
            ...
            <div className="col-span-3 row-start-3 flex w-full items-center gap-2 px-2 sm:col-start-2 sm:col-span-1 sm:row-start-2">
                {showLandscapeButton && onToggleLandscape ? (
                    <button
                        onClick={(e) => {
                            e.stopPropagation();
                            onToggleLandscape();
                        }}
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition-colors ${isLandscape ? (isDaylight ? 'bg-black/10 text-black' : 'bg-white/20') : 'hover:bg-white/10'}`}
                        style={{ color: primaryColor }}
                        title={isLandscape ? t('ui.exitLandscape') : t('ui.enterLandscape')}
                        aria-label={isLandscape ? t('ui.exitLandscape') : t('ui.enterLandscape')}
                    >
                        {isLandscape ? <RotateCcw size={18} /> : <RotateCw size={18} />}
                    </button>
                ) : null}
                ...
                {onHide ? (
                    <button
                        ...
                        className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full opacity-40 transition-colors hover:bg-white/10 hover:opacity-100"
                        style={{ color: primaryColor }}
                        title={t('ui.hideProgressBar')}
                        aria-label={t('ui.hideProgressBar')}
                    >
                        <EyeOff size={16} />
                    </button>
                ) : null}
...
const CollapsedView: React.FC<CollapsedViewProps> = ({
    ...
    const { t } = useTranslation();
    return (
        <div className="flex items-center w-full justify-center gap-2 h-8 px-4">
            <ProgressBar
                currentTime={currentTime}
                duration={duration}
                onSeek={onSeek}
