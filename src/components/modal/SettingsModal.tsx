...
const SettingsModal: React.FC<SettingsModalProps> = ({
    ...
    const {
        ...
        handleSetFusionTuning: onFusionTuningChange,
        handleResetFusionTuning: onResetFusionTuning,
    ...
    return (
        ...
        >
            <motion.div
                {...panelMotion}
                transition={shellTransition}
                className={`${glassBg} border ${borderColor} p-4 md:p-8 landscape:p-3 rounded-3xl ${activeTab === 'options' ? 'w-full md:max-w-4xl md:w-[900px] h-[90vh] md:h-[85vh] landscape:h-[94vh]' : 'w-full md:max-w-lg'} relative shadow-2xl overflow-hidden flex flex-col max-h-[90vh] landscape:max-h-[94vh] transition-all duration-300`}
                onClick={(event) => event.stopPropagation()}
            >
                {/* Decorative background blobs */}
                <div className="absolute inset-0 pointer-events-none z-0">
                            ...
                            */}
                            ...
                            >
                                <div
                                    ...
                                    className={`w-full md:w-1/3 md:max-w-[240px] shrink-0 overflow-x-auto md:overflow-x-hidden md:overflow-y-auto mobile-hide-scrollbar custom-scrollbar pr-0 md:pr-3 flex flex-row md:flex-col space-x-2 md:space-x-0 space-y-0 md:space-y-2 landscape:md:space-y-1 border-b md:border-b-0 md:border-r border-white/10 pb-3 md:pb-4 landscape:md:pb-2 mb-2 md:mb-0 items-center md:items-stretch ${isDragging ? 'cursor-grabbing select-none' : 'cursor-default'}`}
                                >
                                    {SETTINGS_SECTIONS.map((section) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`shrink-0 w-auto md:w-full p-2 md:p-3 landscape:md:p-2 rounded-xl border transition-colors flex items-center justify-center md:justify-between gap-2 md:gap-3 landscape:md:gap-2 text-left ${isActive ? (isDaylight ? 'border-zinc-300/70 bg-white/80' : 'border-white/20 bg-white/10') : (isDaylight ? 'border-transparent hover:bg-white/50' : 'border-transparent hover:bg-white/5')}`}
                                            >
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar pl-1 md:pl-2 pr-2 md:pr-4 landscape:md:pr-2 relative pb-4 landscape:pb-2">
                                    <div className="mb-3 md:mb-6 landscape:mb-2 border-b border-white/10 pb-3 md:pb-4 landscape:pb-2">
                                    ...
                                    <div className="space-y-5 md:space-y-8 landscape:space-y-4">
                                        {activeSettingsSection === 'appearance' && (
            ...
            <AnimatePresence>
                {showVisPlayground && (
                    <VisPlayground
                        ...
                        onFusionTuningChange={onFusionTuningChange}
                        onResetFusionTuning={onResetFusionTuning}
